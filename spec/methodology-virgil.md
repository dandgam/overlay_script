...
- ⬜ **NEW-28 (P0, СТРУКТУРНОЕ) · Тип: 🐛 Баг — DAG слеп к prose-зависимостям реальных
  BMad-историй** — OPEN 2026-05-20, выявлен pilot-прогоном wave 2a. DAG-планировщик
  выбрал `parallel=['1.3','1.4']`, хотя story 1.4 жёстко зависит от 1.3 (1.4 потребляет
  `core/config.py` + `core/db.py`, которые создаёт 1.3). **Корень:** `parse_story_md`
  (`agent/tools/_common.py:210`) понимает только канонический машинный формат
  `- **depends_on:** []` / `- **touches_files:**`. Реальные BMad-истории Antares
  объявляют зависимости русской прозой — `**Зависит от:** 1.1, 1.3 (...)` и
  `**Блокирует:** 1.4 (...)`. Эти строки не матчатся `_FRONTMATTER_RE` → `depends_on`,
  `touches_files`, `touches_shared` всегда пустые → DAG не строит рёбер → планировщик
  считает ВСЕ истории независимыми и параллелит что угодно. Это ломает project-agnostic
  гарантию: на любом реальном BMad-проекте параллелизм небезопасен (неверный порядок +
  невидимые file-collision'ы). **Fix-направление:** парсер BMad-prose story-формата —
  извлекать `**Зависит от:**`/`**Блокирует:**` → `depends_on` (+ обратные рёбра);
  touched-files сложнее (файлы в прозе типа `src/antares/main.py` — нужен консервативный
  regex-эвристик ИЛИ полагаться на `in_flight_touches` mutex + merge-gate). До закрытия
  NEW-28 real-pilot гонять только с явным `--story` (DAG bypass) и вручную проверенным
  порядком. Найдено: pilot wave 2a остановлен на 2-й минуте.
