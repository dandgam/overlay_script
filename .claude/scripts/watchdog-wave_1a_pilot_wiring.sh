#!/bin/bash
# Smart watchdog companion to auto-loop-wave_1a_pilot_wiring.sh.
...
LOG=".claude/watchdog-wave_1a_pilot_wiring.log"
INTEGRATION="integration/wave_1a_pilot_wiring"
...
while true; do
  PID=$(pgrep -f "claude -p /auto-loop-spec spec/spec_wave_1a_pilot_wiring.md" | head -1)
  if [ -z "$PID" ]; then
  ...
  if [ -n "$KILL_REASON" ]; then
    ...
    kill "$PID" 2>/dev/null
    sleep 5
    if pgrep -f "claude -p /auto-loop-spec spec/spec_wave_1a_pilot_wiring.md" >/dev/null; then
      echo "=== $(date -u) escalating to KILL -9" | tee -a "$LOG"
      pkill -9 -f "claude -p /auto-loop-spec spec/spec_wave_1a_pilot_wiring.md"
    fi
