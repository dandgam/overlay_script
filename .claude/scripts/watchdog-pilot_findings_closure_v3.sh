#!/bin/bash
# Smart watchdog companion to auto-loop-pilot_findings_closure_v3.sh.
...
LOG=".claude/watchdog-pilot_findings_closure_v3.log"
INTEGRATION="integration/pilot_findings_closure_v3"
...
while true; do
  PID=$(pgrep -f "claude -p /auto-loop-spec spec/spec_pilot_findings_closure_v3.md" | head -1)
  if [ -z "$PID" ]; then
  ...
  if [ -n "$KILL_REASON" ]; then
    ...
    kill "$PID" 2>/dev/null
    sleep 5
    if pgrep -f "claude -p /auto-loop-spec spec/spec_pilot_findings_closure_v3.md" >/dev/null; then
      echo "=== $(date -u) escalating to KILL -9" | tee -a "$LOG"
      pkill -9 -f "claude -p /auto-loop-spec spec/spec_pilot_findings_closure_v3.md"
    fi
