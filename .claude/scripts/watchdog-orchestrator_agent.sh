#!/bin/bash
# Smart watchdog companion to auto-loop-orchestrator_agent.sh.
# Kills earlier than 1h `timeout` ceiling when commit done but process is hung.

LOG=".claude/watchdog-orchestrator_agent.log"
...
while true; do
  PID=$(pgrep -f "claude -p /auto-loop-spec spec/spec_orchestrator_agent.md" | head -1)
  if [ -z "$PID" ]; then
  ...
  if [ -n "$KILL_REASON" ]; then
    ...
    kill "$PID" 2>/dev/null
    sleep 5
    if pgrep -f "claude -p /auto-loop-spec spec/spec_orchestrator_agent.md" >/dev/null; then
      echo "=== $(date -u) escalating to KILL -9" | tee -a "$LOG"
      pkill -9 -f "claude -p /auto-loop-spec spec/spec_orchestrator_agent.md"
    fi
