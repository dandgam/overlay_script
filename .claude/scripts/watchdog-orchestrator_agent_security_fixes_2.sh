#!/bin/bash
LOG=".claude/watchdog-orchestrator_agent_security_fixes_2.log"
...
while true; do
  PID=$(pgrep -f "claude -p /auto-loop-spec spec/spec_orchestrator_agent_security_fixes_2.md" | head -1)
  ...
  if [ -n "$KILL_REASON" ]; then
    ...
    sleep 2; kill "$PID" 2>/dev/null; sleep 5
    pgrep -f "claude -p /auto-loop-spec spec/spec_orchestrator_agent_security_fixes_2.md" >/dev/null && \
      pkill -9 -f "claude -p /auto-loop-spec spec/spec_orchestrator_agent_security_fixes_2.md"
  fi
  sleep "$INTERVAL"
done
