#!/bin/bash
LOG=".claude/watchdog-orchestrator_agent_security_fixes_4.log"
...
while true; do
  PID=$(pgrep -f "claude -p /auto-loop-spec spec/spec_orchestrator_agent_security_fixes_4.md" | head -1)
  ...
  if [ -n "$KILL_REASON" ]; then
    ...
    pgrep -f "claude -p /auto-loop-spec spec/spec_orchestrator_agent_security_fixes_4.md" >/dev/null && \
      pkill -9 -f "claude -p /auto-loop-spec spec/spec_orchestrator_agent_security_fixes_4.md"
  fi
  sleep "$INTERVAL"
done
