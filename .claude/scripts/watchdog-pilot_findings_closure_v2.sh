#!/bin/bash
# Smart watchdog companion to auto-loop-pilot_findings_closure_v2.sh.
...
LOG=".claude/watchdog-pilot_findings_closure_v2.log"
...
while true; do
  ...
  if [ -n "$KILL_REASON" ]; then
    ...
    kill "$PID" 2>/dev/null
    sleep 5
    if pgrep -f "claude -p /auto-loop-spec spec/spec_pilot_findings_closure_v2.md" >/dev/null; then
      echo "=== $(date -u) escalating to KILL -9" | tee -a "$LOG"
      pkill -9 -f "claude -p /auto-loop-spec spec/spec_pilot_findings_closure_v2.md"
    fi
