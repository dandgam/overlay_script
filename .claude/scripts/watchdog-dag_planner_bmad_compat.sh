#!/bin/bash
LOG=".claude/watchdog-dag_planner_bmad_compat.log"
...
while true; do
  PID=$(pgrep -f "claude -p /auto-loop-spec spec/spec_dag_planner_bmad_compat.md" | head -1)
  if [ -z "$PID" ]; then
  ...
  if [ -n "$KILL_REASON" ]; then
    ...
    kill "$PID" 2>/dev/null
    sleep 5
    if pgrep -f "claude -p /auto-loop-spec spec/spec_dag_planner_bmad_compat.md" >/dev/null; then
      pkill -9 -f "claude -p /auto-loop-spec spec/spec_dag_planner_bmad_compat.md"
    fi
  fi
  sleep "$INTERVAL"
done
