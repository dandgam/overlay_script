#!/bin/bash
LOG=".claude/watchdog-embed_phase45_fixes.log"
...
while true; do
  PID=$(pgrep -f "claude -p /auto-loop-spec spec/spec_embed_phase45_fixes.md" | head -1)
  ...
  elif [ "$PROC_AGE" -gt "$MAX_PROCESS_AGE" ] && [ "$COMMIT_AGE" -gt "$MAX_COMMIT_STALE" ]; then KILL_REASON="post-commit zombie"
  fi
  if [ -n "$KILL_REASON" ]; then
    echo "=== $(date -u) KILL: PID=$PID — $KILL_REASON" | tee -a "$LOG"
    pkill -P "$PID" 2>/dev/null; sleep 2; kill "$PID" 2>/dev/null; sleep 5
    if pgrep -f "claude -p /auto-loop-spec spec/spec_embed_phase45_fixes.md" >/dev/null; then
      pkill -9 -f "claude -p /auto-loop-spec spec/spec_embed_phase45_fixes.md"
    fi
  fi
  sleep "$INTERVAL"
done
