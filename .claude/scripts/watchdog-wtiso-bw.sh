#!/bin/bash
# Smart watchdog companion to auto-loop-wtiso-bw.sh.
...
# pgrep polls). Kills earlier than the 1h `timeout` ceiling when this exact pattern is detected.

LOG=".claude/watchdog-wtiso-bw.log"
INTEGRATION="integration/wtiso-bw"
MAX_PROCESS_AGE=3000     # 50 min
MAX_COMMIT_STALE=2100    # 35 min
HARD_CEILING_AGE=5400    # 90 min
INTERVAL=60

cd /home/server/bmad-orchestrator || exit 1

while true; do
  PID=$(pgrep -f "claude -p /auto-loop-spec spec/spec_wtiso-bw.md" | head -1)
  if [ -z "$PID" ]; then
  ...
  if [ -n "$KILL_REASON" ]; then
    ...
    kill "$PID" 2>/dev/null
    sleep 5
    if pgrep -f "claude -p /auto-loop-spec spec/spec_wtiso-bw.md" >/dev/null; then
      echo "=== $(date -u) escalating to KILL -9" | tee -a "$LOG"
      pkill -9 -f "claude -p /auto-loop-spec spec/spec_wtiso-bw.md"
    fi
