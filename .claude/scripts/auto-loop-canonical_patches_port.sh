...
LOG=".claude/auto-loop-canonical_patches_port.log"
DELAY=120
...
while true; do
  ...
  if grep -q '^## Final Report$' "$TRACKER" && \
    ...
    if [ "$FINAL_LEN" -ge 80 ]; then echo "=== $(date -u) Final Report populated, exiting ===" | tee -a "$LOG"; break; fi
  fi
  ...
  RC=$?
  if [ "$RC" -eq 124 ]; then echo "=== $(date -u) WARNING: timeout ===" | tee -a "$LOG"
  elif [ "$RC" -ne 0 ]; then echo "=== $(date -u) NOTE: rc=$RC ===" | tee -a "$LOG"; fi
