#!/bin/bash
LOG=".claude/watchdog-embed_phase45_with_selflearning.log"
...
while true; do
  PID=$(pgrep -f "claude -p /auto-loop-spec spec/spec_embed_phase45_with_selflearning.md" | head -1)
  ...
  if [ -n "$KILL_REASON" ]; then
    ...
    kill "$PID" 2>/dev/null
    sleep 5
    if pgrep -f "claude -p /auto-loop-spec spec/spec_embed_phase45_with_selflearning.md" >/dev/null; then
      pkill -9 -f "claude -p /auto-loop-spec spec/spec_embed_phase45_with_selflearning.md"
    fi
  fi
  sleep "$INTERVAL"
done
