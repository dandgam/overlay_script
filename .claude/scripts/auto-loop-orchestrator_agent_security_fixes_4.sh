...
TRACKER=".claude/initiative-tracker-orchestrator_agent_security_fixes_4.md"
LOG=".claude/auto-loop-orchestrator_agent_security_fixes_4.log"
DELAY=600
SPEC="spec/spec_orchestrator_agent_security_fixes_4.md"
INTEGRATION="integration/orchestrator_agent_security_fixes_4"
PROMOTE_SCRIPT=".claude/scripts/auto-promote-tracker.py"

while true; do
  if [ ! -f "$TRACKER" ]; then
    echo "=== $(date -u) FATAL: tracker missing ===" | tee -a "$LOG"; exit 1
  fi
  ...
  if grep -q '^## Final Report$' "$TRACKER" && \
    ...
    if [ "$FINAL_LEN" -ge 80 ]; then
      echo "=== $(date -u) Final Report populated (${FINAL_LEN} chars) — done, exiting ===" | tee -a "$LOG"; break
    fi
  fi
  ...
  if [ -x "$PROMOTE_SCRIPT" ]; then
    ...
    if [ "$PROMOTE_RC" -eq 0 ]; then
      echo "=== $(date -u) zombie recovered ===" | tee -a "$LOG"; continue
    fi
  fi

  echo "=== $(date -u) starting session (fresh context, base=integration/orchestrator_agent_security_fixes_3) ===" | tee -a "$LOG"
  timeout 3600 claude -p "/auto-loop-spec $SPEC" --model opus --dangerously-skip-permissions 2>&1 | tee -a "$LOG"
  RC=$?
  if [ "$RC" -eq 124 ]; then
    echo "=== $(date -u) WARNING: 1h timeout ===" | tee -a "$LOG"
  elif [ "$RC" -ne 0 ]; then
