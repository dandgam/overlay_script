#!/bin/bash
# Auto-loop wrapper for orchestrator_agent_security_fixes (manual bootstrap, 2026-05-16).
# Spec: spec/spec_orchestrator_agent_security_fixes.md
# Slug: orchestrator_agent_security_fixes
# Base: integration/orchestrator_agent (НЕ main)
# Delay between sessions: 600s
# Stop conditions:
#   - Final Report appears in tracker → break
#   - 'resolution: PENDING' in last 120 lines → break
# Manual stop: tmux kill-session -t autoloop-orchestrator_agent_security_fixes

set -u
cd /home/server/bmad-orchestrator || exit 1

TRACKER=".claude/initiative-tracker-orchestrator_agent_security_fixes.md"
LOG=".claude/auto-loop-orchestrator_agent_security_fixes.log"
DELAY=600
SPEC="spec/spec_orchestrator_agent_security_fixes.md"
INTEGRATION="integration/orchestrator_agent_security_fixes"
...
while true; do
  ...
  if [ -x "$PROMOTE_SCRIPT" ]; then
    ...
    if [ "$PROMOTE_RC" -eq 0 ]; then
      echo "=== $(date -u) zombie recovered by auto-promote-tracker.py — re-checking ===" | tee -a "$LOG"
      continue
    fi
  fi

  echo "=== $(date -u) starting session (fresh context, runtime=loop_wrapper, base=integration/orchestrator_agent) ===" | tee -a "$LOG"
