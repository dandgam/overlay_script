#!/bin/bash
# Smart watchdog companion to auto-loop-pilot_findings_closure.sh.
# Kills earlier than the 1h `timeout` ceiling when commit is done but process is hung.

LOG=".claude/watchdog-pilot_findings_closure.log"
INTEGRATION="integration/pilot_findings_closure"
...
while true; do
  PID=$(pgrep -f "claude -p /auto-loop-spec spec/spec_pilot_findings_closure.md" | head -1)
  if [ -z "$PID" ]; then
  ...
  if [ -n "$KILL_REASON" ]; then
    ...
    kill "$PID" 2>/dev/null
    sleep 5
    if pgrep -f "claude -p /auto-loop-spec spec/spec_pilot_findings_closure.md" >/dev/null; then
      echo "=== $(date -u) escalating to KILL -9" | tee -a "$LOG"
      pkill -9 -f "claude -p /auto-loop-spec spec/spec_pilot_findings_closure.md"
    fi
