...
## ⚠️ Instructions for Claude (worker reading this)

**Do exactly this — do NOT verify paths or layouts yourself, do NOT ask the user:**

```bash
bash scripts/bmad-auto-dev-runner.sh "$@"
```

The runner (`scripts/bmad-auto-dev-runner.sh`) does ALL pre-flight checks itself:
- **Layout auto-detect (Patch Y 2026-05-18)** — supports stock BMM v6 (`_bmad/output/planning/`) AND Odyssey hybrid (`_bmad/planning-artifacts/`) AND env-var override (`BMAD_EPICS_FILE`, etc.). Runner picks the right one; you don't need to.
- **Story ID + branch name** — runner accepts any current feature branch (`feature/<anything>`) and any story-id format (long-slug or short `1.1`). No translation needed.
- **Missing artifacts** — runner exits 1 with a clear error if anything is genuinely missing.

The paths shown below (`_bmad/planning-artifacts/...`, `_bmad/stories/<id>.md`, `feature/story-<id>`) are **historical Odyssey-hybrid examples** — they document the ORIGINAL layout but the runner now handles all variants. When you (Claude) see those paths, mentally substitute the runner's auto-detected `${EPICS_FILE}` / `${STORIES_DIR}/${id}.md` / current branch.

**You will be tempted to "verify alignment" before running. Resist that urge.** Just `bash scripts/bmad-auto-dev-runner.sh` and let it speak. If it fails, parse its stderr — do not pre-emptively diagnose.
...
# Per BMad-project (must exist) — runner auto-detects layout:
#   Stock BMM v6:        _bmad/output/planning/epics.md
#                        _bmad/output/planning/stories/sprint-status.yaml
#   Odyssey hybrid:      _bmad/planning-artifacts/epics.md
#                        _bmad/implementation-artifacts/sprint-status.yaml
#   Override (env vars): BMAD_EPICS_FILE, BMAD_SPRINT_STATUS, BMAD_STORIES_DIR, BMAD_PLANNING_DIR, BMAD_IMPL_DIR

# Created by skill per run (path depends on detected layout):
<project>/<STORIES_DIR>/<id>.md                  # populated by Stage 4 create-story
...
- Auto-detect BMad layout (Patch Y 2026-05-18) — sets `EPICS_FILE`, `SPRINT_STATUS`, `STORIES_DIR`, `PLANNING_DIR`, `IMPL_DIR`:
  - Env vars `BMAD_EPICS_FILE` + `BMAD_SPRINT_STATUS` + `BMAD_STORIES_DIR` override everything.
  - Else if `_bmad/output/planning/` exists → stock BMM v6 layout.
  - Else if `_bmad/planning-artifacts/` exists → Odyssey hybrid layout.
  - Else abort exit 1.
- Verify resolved `EPICS_FILE` + `SPRINT_STATUS` exist. Abort exit 1 if missing.
