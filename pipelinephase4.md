...
#### S5 внутри — скилл `bmad-dev-story` (485 строк): активация + 10 шагов

**Промпт раннера ≠ «выполни скилл»** (`runner:449-455`) — в нём 4 добавки-урока: ① точная ссылка `Read _bmad/stories/<id>.md`; ② `Commit on branch feature/story-<id>`; ③ `CRITICAL: cargo check --workspace` до «готово» (урок Story 1.11); ④ новый crate → обязательно `Cargo.toml + lib.rs/main.rs + mod.rs` (тот же класс «полфайла забыл»); + working dir.

**Фаза А — активация (6 шагов, как у S4):** resolve_customization (3 слоя: база → `_bmad/custom/bmad-dev-story.toml` **нет** → `.user.toml` нет) → prepend (пусто) → **persistent_facts**: `file:{project-root}/**/project-context.md` — вот как Sonnet с пустой памятью получает стандарты проекта → config.yaml (язык/имя/skill level) → greet → append (пусто). `on_complete` hook пуст.

**Фаза Б — workflow (10 шагов):**

| шаг | что | конвейерные нюансы |
|---|---|---|
| ① | найти + прочитать историю; нет задач → ⑨ | id передан явно → ветки «спроси пользователя» (`:108-186`) **недостижимы** (наследие ручного режима, как в S4) |
| ② | контекст проекта (Dev Notes, стандарты) | — |
| ③ | **детект продолжения**: секция «Senior Developer Review (AI)» есть? | да → сначала чинить `[AI-Review]`-замечания (High→Low). **Один скилл = две роли**: fresh dev И починка после ревью (S6.retry переиспользует) |
| ④ | sprint-status: `ready-for-dev → in-progress` | flip статуса №1 (из трёх за жизнь story) |
| ⑤ | задача N: 🔴RED → 🟢GREEN → 🔵REFACTOR | строго по порядку Tasks, никакой отсебятины (`:298`, `:322`) |
| ⑥ | дописать тесты unit/integration/e2e | эвристики покрытия (`:328-332`) |
| ⑦ | прогнать ВСЁ: регресс + новые + lint + сверка AC | регресс упал → STOP и чинить (`:341`) |
| ⑧ | гейты честности → `[x]` ТОЛЬКО если реально зелено | «NO LYING OR CHEATING» (`:346`); ещё задачи → LOOP:⑤ |
| ⑨ | финал: все `[x]` + полный регресс + DoD-чеклист (11 пунктов `:404-416`) | `Status="review"` (flip №2); 4 финальных HALT-гейта (`:441-444`) |
| ⑩ | отчёт человеку + on_complete | в headless улетает в лог; «совет: ревью другой LLM» — конвейер так и делает (Opus на S6) |

**Право записи — только 5 зон** (`:13`): чекбоксы Tasks/Subtasks, Dev Agent Record, File List, Change Log, Status. Текст истории и AC **неприкосновенны** — исполнитель не может «подправить требование под код»; AC остаются чистым эталоном для Acceptance Auditor (S6).

**Анти-стоп директивы** (`:15-16`): «не останавливайся на milestones / significant progress / session boundaries», «только шаг 9 решает что готово» — противоядие от вежливых пауз LLM; в headless спросить некого, пауза = зависший конвейер.
